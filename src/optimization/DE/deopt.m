...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [FVr_bestmem,S_bestval,I_nfeval, History, HistoryFit] = deopt(fname,S_struct)
...
% for k=1:I_NP
   FM_pop = FVr_minbound + rand(I_NP,I_D).*(FVr_maxbound - FVr_minbound);
% end
...
I_iter = 1;
History = zeros(I_itermax, I_D);
HistoryFit = zeros(1, I_D);
while ((I_iter < I_itermax) && (S_bestval > F_VTR))
...
%     FM_ui = FM_bm + F_weight*(FM_pm1 - FM_pm2); %without jitter             
    FM_ui = FM_popold.*FM_mpo + FM_ui.*FM_mui;
    FM_origin = FM_bm;
  elseif (I_strategy == 4)                         % DE/best/1
    FM_ui = FM_bm + F_weight*(FM_pm1 - FM_pm2); %without jitter             
    FM_ui = FM_popold.*FM_mpo + FM_ui.*FM_mui;
    FM_origin = FM_bm;
...
%      FM_ui = FM_popold.*FM_mpo + FM_ui.*FM_mui;     % crossover
  ...
  elseif (I_strategy == 5)                          % DE/rand/2
      FM_ui = FM_pm3 + F_weight*(FM_pm1 - FM_pm2)+F_weight*(FM_pm4 - FM_pm5);
      FM_ui = FM_popold.*FM_mpo + FM_ui.*FM_mui;     % crossover
      FM_origin = FM_pm3;
  elseif (I_strategy == 6)                          % DE/best/2
      FM_ui = FM_bm + F_weight*(FM_pm1 - FM_pm2)+F_weight*(FM_pm4 - FM_pm3);
      FM_ui = FM_popold.*FM_mpo + FM_ui.*FM_mui;     % crossover
      FM_origin = FM_pm3;
...
%-----Select which vectors are allowed to enter the new population------------
    ...
    if (I_bnd_constr == 1)
         FM_ui(FM_ui<FVr_minbound) = FVr_minbound + rand*(FM_origin(FM_ui<FVr_minbound) - FVr_minbound);
         FM_ui(FM_ui>FVr_maxbound) = FVr_maxbound + rand*(FM_origin(FM_ui>FVr_maxbound) - FVr_maxbound);
    end
      %=====End boundary constraints==========================================
    S_new_fvalues = feval(fname,FM_ui,S_struct);
    I_nfeval  = I_nfeval + I_NP;
    change_particles = S_new_fvalues <= S_val;
    FM_pop(change_particles,:) = FM_ui(change_particles,:);
    S_val(change_particles,:) = S_new_fvalues(change_particles,:);
    [S_bestval FM_bestInd] = min(S_val);
    FVr_bestmem = FM_pop(FM_bestInd,:);
...
%        end
  ...
  History(I_iter,:) = FVr_bestmem;
  HistoryFit(I_iter) = S_bestval;
