...
sigma_ax  = sigma .* randn(size(control_points(:,1:2)));
sigma_sag = sigma .* randn(size(control_points(:,1:2)));
sigma_cor = sigma .* randn(size(control_points(:,1:2)));

deform_points_ax  = control_points(:,1:2) + sigma_ax;
deform_points_sag = control_points(:,2:3) + sigma_sag;
deform_points_cor = [control_points(:,1) control_points(:,3)] + sigma_cor;
...
for k = 1:21
    ...
    disp(['--------------- Deformation ', num2str(k),'--------------------------------------'])
    save_name  = strcat('deformation_noise_',num2str(k),'.mat')
    save_name2 = strcat('result_deformation_noise_',num2str(k),'.mat');
    ...
    save(save_name,'opt_im_ax','opt_im_sag','opt_im_cor','deform_tri_ax','deform_tri_sag','deform_tri_cor',...
          'var_array1_v','var_array2_v','var_array3_v','sigma_ax','sigma_sag','sigma_cor',...
          'var_cell1_v', 'var_cell2_v', 'var_cell3_v');
      
   main_evaluation(save_name2, dcmdir, opt_im_ax, opt_im_sag, opt_im_cor);
