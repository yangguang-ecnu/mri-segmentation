 ...
 *  2011/05/23: Multiple output arguments
 *
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    mwSize ndims_u = mxGetNumberOfDimensions(prhs[2]);
    const mwSize* dims_u = mxGetDimensions(prhs[2]);
    ...
    if (nlhs == 0) 
        ...
        if (nlhs != bbs.valdim)
            mexErrMsgTxt("The number of output arguments must be equal to 1 or to the dimension of the B-spline.");
        
        // ACTUAL EVALUATION
        double *val = (double*)mxMalloc(bbs.valdim*nval_u*sizeof(double));
        eval(&bbs, ctrlpts, u, v, nval_u, val, du, dv);
        
        // Allocation of the output arguments
        double **val_cur_dim = (double**)mxMalloc(bbs.valdim*sizeof(double));
        for (int i = 0 ; i < bbs.valdim ; ++i) {
            plhs[i] = mxCreateNumericArray(ndims_u, dims_u, mxDOUBLE_CLASS, mxREAL);
            val_cur_dim[i] = mxGetPr(plhs[i]);
        }
        
        // Copy the result into the output arguments
        int k = 0;
        for (int j = 0 ; j < nval_u ; ++j)
            for (int i = 0; i < bbs.valdim; ++i) {
                val_cur_dim[i][j] = val[k];
                ++k;
            }
        mxFree(val);
        mxFree(val_cur_dim);
