function patient = parseDicomdir(fname,folder)
	...
	for i = 1:nFields
		...
		switch lower(record.DirectoryRecordType)
			...
			case 'image'
                ...
                file_name = strcat(folder,patient{nPatients}.study{nStudies}.series{nSeries}.image{nImages}.info.ReferencedFileID);
                im = dicom(file_name,0);
                patient{nPatients}.study{nStudies}.series{nSeries}.image{nImages}.image = im;
			otherwise
